...
<Steps>
  ...
  <Step title="Optional: create a filter">
    Instead of forwarding _all_ email, create a filter so only relevant messages are forwarded:
    - On the same **Forwarding and POP/IMAP** page, click the **"creating a filter"** link (in the tip text).
    - Set criteria such as **From** `billing@vendor.com` or **Subject** contains `Invoice`.
