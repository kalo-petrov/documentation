...
<Info>
  **Using the global sequence instead?** For most users the better option is to configure the [global Invoice Numbering setting](/invoices/invoice-numbering) under Invoices → Invoice Numbering. That makes `{global_invoice_number}` available automatically inside every **Create Invoice** step — no separate numbering step needed. Use this step when you need a **secondary** or **automation-specific** sequence alongside the global one.
</Info>
...
|---|---|---|
| **Variable** | `{invoice_number_local}` (default, customisable) | `{global_invoice_number}` |
